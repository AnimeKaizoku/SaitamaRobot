...
def runban(update: Update, context: CallbackContext):
  ...
  try:
    ...
    if excp.message == "Reply message not found":
      ...
      LOGGER.warning(update)
      LOGGER.exception("ERROR unbanning user %s in chat %s (%s) due to %s",
                       user_id, chat.title, chat.id, excp.message)
...
def rmute(update: Update, context: CallbackContext):
  ...
  try:
    ...
    if excp.message == "Reply message not found":
      ...
      LOGGER.warning(update)
      LOGGER.exception("ERROR mute user %s in chat %s (%s) due to %s", user_id,
...
def runmute(update: Update, context: CallbackContext):
  ...
  try:
    ...
    if excp.message == "Reply message not found":
      ...
      LOGGER.warning(update)
      LOGGER.exception("ERROR unmnuting user %s in chat %s (%s) due to %s",
                       user_id, chat.title, chat.id, excp.message)
