...
def new_fed(bot: Bot, update: Update):
    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text("Failed to create federation! Head over to @OnePunchSupport to notify us of the error.")
            return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
                message.reply_text("Failed to join federation! Please head to @OnePunchSupport to report this.")
                return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if fban:
        ...
        if not x:
            message.reply_text("Failed to ban from the federation! If this problem persists, reach out to us @OnePunchSupport.")
            return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text("Failed to set federation rules. If this persists, reach out to us @OnePunchSupport.")
            return
